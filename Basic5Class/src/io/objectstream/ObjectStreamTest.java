		...
		public void actionPerformed( ActionEvent ev ){
			...
			if( evtBtn == bSave )
			{
				...
				*/
				 ...
				 * */
				try{
					ObjectOutputStream oos = new ObjectOutputStream(	//객체를 통으로 쓸수 있는 통로 만듬
							new FileOutputStream("data2.txt"));
					Person p = new Person();	//name,age,height,bloodType을 통으로 person p를 만듬
					p.setName(name);
					p.setAge(age);
					p.setHeight(height);
			...
			*/
				try{
					...
					ObjectInputStream ois = new ObjectInputStream(	//객체를 통으로 쓸수 있는 통로 만듬
